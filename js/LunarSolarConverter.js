// js/LunarSolarConverter.js (已修正 LunarToSolar 函數中的閏月計算邏輯)
...
function LunarSolarConverter() {
    ...
    this.LunarToSolar = function (lunar) {
        // 【已修正此函數的閏月邏輯】
        const lunarYearIndex = lunar.lunarYear - this.lunar_month_days[0];
        if (lunarYearIndex < 0 || lunarYearIndex >= this.lunar_month_days.length -1) {
             return new Solar(); // Return empty object if year is out of range
        }
        
        var days = this.lunar_month_days[lunarYearIndex];
        var leap = this.GetBitInt(days, 4, 13);
        var offset = 0;

        // 計算累計天數
        for (var i = 1; i < lunar.lunarMonth; i++) {
            // 如果有閏月，且當前循環的月份是閏月，則加上閏月的天數
            if (leap !== 0 && i === leap) {
                offset += this.GetBitInt(days, 1, 12 - (i-1)) === 1 ? 30 : 29;
            }
            offset += this.GetBitInt(days, 1, 12 - (i-1)) === 1 ? 30 : 29;
        }

        // 如果當前所求的月份是閏月，要額外加上正常月份的天數
        if (lunar.isleap) {
            if (lunar.lunarMonth === leap) {
                 offset += this.GetBitInt(days, 1, 12 - (lunar.lunarMonth-1)) === 1 ? 30 : 29;
            } else {
                 return new Solar(); // Invalid leap month
            }
        }

        offset += lunar.lunarDay;

        var solar11 = this.solar_1_1[lunarYearIndex];
        var y = this.GetBitInt(solar11, 12, 9);
        var m = this.GetBitInt(solar11, 4, 5);
        var d = this.GetBitInt(solar11, 5, 0);

        if (y === 0) { return new Solar(); } // 如果查不到農曆新年的陽曆日期
    ...
    this.SolarToLunar = function (solar) {
        ...
        if (index < 0 || index >= this.solar_1_1.length -1) {
            return new Lunar(); // Return empty if out of range
        }
        ...
        var leap = this.GetBitInt(days, 4, 13);
        lunar.lunarYear = index + this.solar_1_1[0];
        
        var lunarM = 1;
        offset++;

        for (var i = 1; i <= 13; i++) {
            var isLeapMonthNow = (leap !== 0 && i === leap);
            var monthDays;

            if (isLeapMonthNow) {
                monthDays = this.GetBitInt(days, 1, 12) === 1 ? 30 : 29; // 閏月天數存在 bit 12 (假設)
            } else {
                 // 正常月份，根據其位置計算 bit
                let monthBitIndex = i;
                if(leap !== 0 && i > leap) monthBitIndex = i - 1;
                monthDays = this.GetBitInt(days, 1, 12 - (monthBitIndex-1) ) === 1 ? 30 : 29;
            }
        ...
        lunar.lunarDay = parseInt(offset);
        return lunar;
    }
}